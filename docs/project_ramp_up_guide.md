...
- The TFT currently handles:
  - fast RaceSense boot wordmark from a raw RGB565 asset streamed by `boot.py`
...
- The boot wordmark is generated by `firmware/tools/generate_tft_wordmark.swift` into `firmware/lib/tft_wordmark.raw` and streamed directly to the TFT. `TFTBootUI` skips reset/clear/re-init so `main.py` does not introduce a white flash after the early boot draw.
