...
- The native firmware supports the Waveshare 4.3" display as its production baseline and the 2.8" TFT + touch panel through the same target-agnostic BSP under `firmware/`.
...
- The TFT currently handles:
  ...
  - fast RaceSense boot splash from the native embedded RGB565 asset once display initialization is complete
...
- Native LVGL typography and the embedded RaceSense splash are selected by the ESP-IDF display target; there is no Python font or runtime asset sync step.
