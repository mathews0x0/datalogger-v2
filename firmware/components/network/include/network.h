...
/** Server heartbeat lifecycle reported during a cloud sync. */
typedef enum {
    NETWORK_HEARTBEAT_PINGING,
    NETWORK_HEARTBEAT_ACKNOWLEDGED,
    NETWORK_HEARTBEAT_FAILED,
} network_heartbeat_event_t;

/** Callback invoked around the authenticated cloud heartbeat request. */
typedef void (*network_heartbeat_cb_t)(network_heartbeat_event_t event,
                                       void *user_ctx);
...
/**
 * @brief Upload pending sessions while exposing heartbeat state to the UI.
 *
 * The heartbeat callback is called immediately before the server ping,
 * after the server acknowledges it, or after the ping fails.
 */
