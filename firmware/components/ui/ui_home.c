...
/* Generated from the same full logo used by server/ui/index.html. */
extern const uint8_t _binary_racesense_splash_rgb565_bin_start[];

#define BOOT_LOGO_WIDTH  640
#define BOOT_LOGO_HEIGHT 230
#define BOOT_LOGO_BYTES  (BOOT_LOGO_WIDTH * BOOT_LOGO_HEIGHT * 2)

static const lv_img_dsc_t s_boot_logo = {
    .header.cf = LV_IMG_CF_TRUE_COLOR,
    .header.w = BOOT_LOGO_WIDTH,
    .header.h = BOOT_LOGO_HEIGHT,
    .data_size = BOOT_LOGO_BYTES,
    .data = _binary_racesense_splash_rgb565_bin_start,
};
...
void ui_show_boot_splash(void)
{
    ...
    lv_obj_clean(scr);
    /* Match the server landing page's #050508 canvas, rather than the
     * dashboard ground used by Home. */
    lv_obj_set_style_bg_color(scr, C(0x050508), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(scr, LV_OPA_COVER, LV_PART_MAIN | LV_STATE_DEFAULT);

    lv_obj_t *rail = lv_obj_create(scr);
    lv_obj_set_size(rail, UI_PCT_X(88), UI_SCALE_Y(2));
    lv_obj_align(rail, LV_ALIGN_TOP_MID, 0, UI_SCALE_Y(52));
    lv_obj_set_style_bg_color(rail, C(UI_COLOR_PRIMARY), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(rail, LV_OPA_COVER, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_width(rail, 0, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_radius(rail, 0, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_clear_flag(rail, LV_OBJ_FLAG_SCROLLABLE);

#if UI_RES_CLASS_WIDESCREEN
    lv_obj_t *logo = lv_img_create(scr);
    lv_img_set_src(logo, &s_boot_logo);
    lv_obj_align(logo, LV_ALIGN_CENTER, 0, -UI_SCALE_Y(72));
#else
    /* Keep the smaller profiles lightweight while retaining the server palette. */
    lv_obj_t *logo = lv_label_create(scr);
    lv_label_set_text(logo, "RaceSense");
    label_style(logo, UI_COLOR_TEXT_PRIMARY, &lv_font_montserrat_28);
    lv_obj_align(logo, LV_ALIGN_CENTER, 0, -UI_SCALE_Y(38));
#endif

    lv_obj_t *bar = lv_bar_create(scr);
    lv_obj_set_size(bar, UI_PCT_X(55), UI_SCALE_Y(10));
    lv_bar_set_range(bar, 0, 100);
    lv_bar_set_value(bar, 70, LV_ANIM_OFF);
    lv_obj_set_style_bg_color(bar, C(UI_COLOR_BORDER), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_color(bar, C(UI_COLOR_PRIMARY), LV_PART_INDICATOR | LV_STATE_DEFAULT);
    ...
    lv_obj_align(bar, LV_ALIGN_CENTER, 0, UI_SCALE_Y(140));

    lv_obj_t *detail = lv_label_create(scr);
    lv_label_set_text(detail, "PREPARING TELEMETRY ENGINE");
    label_style(detail, UI_COLOR_TEXT_MUTED, font_body());
    lv_obj_align(detail, LV_ALIGN_CENTER, 0, UI_SCALE_Y(173));

    lv_obj_t *tagline = lv_label_create(scr);
    lv_label_set_text(tagline, "RIDE FASTER  /  RIDE SMARTER");
    label_style(tagline, UI_COLOR_PRIMARY, font_small());
    lv_obj_align(tagline, LV_ALIGN_CENTER, 0, UI_SCALE_Y(202));

    /* Paint the composed splash while the panel is still dark. The BSP turns
     * the backlight on only after this synchronous refresh completes. */
    lv_refr_now(NULL);
    ui_load_screen(scr);
    ui_unlock();
}
