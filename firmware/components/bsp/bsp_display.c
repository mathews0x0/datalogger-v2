...
static bool s_backlight_configured;
#endif

void bsp_display_backlight_on(void)
{
#if BSP_DISPLAY_TARGET == 0
    if (!s_backlight_configured) {
        return;
    }
    /* GPIO26 is active-low: 1023 with the inverted LEDC output is full on. */
...
/* GT911s on this panel are occasionally still starting up when the P4
 * application reaches BSP init, especially after a USB flash/reset.  Keep
 * the recovery timings here rather than relying on the shorter reset pulse
 * inside the generic component. */
#define GT911_INIT_ATTEMPTS       4
#define GT911_RESET_LOW_MS        20
#define GT911_STARTUP_WAIT_MS     120
#define GT911_RETRY_WAIT_MS       80
...
static esp_err_t gt911_hardware_reset(void)
{
    ...
    const gpio_config_t rst_cfg = {
        .pin_bit_mask = BIT64(BSP_TOUCH_RST_GPIO),
        .mode = GPIO_MODE_OUTPUT,
        .pull_up_en = GPIO_PULLUP_DISABLE,
        .pull_down_en = GPIO_PULLDOWN_DISABLE,
        .intr_type = GPIO_INTR_DISABLE,
    };
    ESP_RETURN_ON_ERROR(gpio_config(&rst_cfg), TAG, "GT911 reset GPIO config failed");
    ESP_RETURN_ON_ERROR(gpio_set_level(BSP_TOUCH_RST_GPIO, 0), TAG,
                        "GT911 reset assertion failed");
    vTaskDelay(pdMS_TO_TICKS(GT911_RESET_LOW_MS));
    ESP_RETURN_ON_ERROR(gpio_set_level(BSP_TOUCH_RST_GPIO, 1), TAG,
                        "GT911 reset release failed");
    vTaskDelay(pdMS_TO_TICKS(GT911_STARTUP_WAIT_MS));
    return ESP_OK;
}
...
esp_err_t bsp_display_driver_init(void)
{
    ...
    /* Backlight is active-low PWM on GPIO26. Keep it physically off while
     * the panel and the first LVGL frame are being initialized. */
...
esp_err_t bsp_touch_driver_init(void)
{
     ...
     * Reset before probing: after flashing, the controller can otherwise be
     * left in its boot/configuration window and NACK the first transaction. */
    esp_lcd_panel_io_i2c_config_t tp_io_cfg = ESP_LCD_TOUCH_IO_I2C_GT911_CONFIG();
    esp_err_t probe_ret = ESP_ERR_NOT_FOUND;
    for (int attempt = 1; attempt <= GT911_INIT_ATTEMPTS; ++attempt) {
        ESP_RETURN_ON_ERROR(gt911_hardware_reset(), TAG, "GT911 hardware reset failed");
        /* Recover SDA/SCL if a reset interrupted a transaction in progress. */
        esp_err_t bus_ret = i2c_master_bus_reset(s_touch_i2c_bus);
        if (bus_ret != ESP_OK) {
            ESP_LOGW(TAG, "GT911 I2C bus reset %d/%d failed: %s", attempt,
                     GT911_INIT_ATTEMPTS, esp_err_to_name(bus_ret));
        }
        ...
        ESP_LOGW(TAG, "GT911 probe attempt %d/%d failed", attempt, GT911_INIT_ATTEMPTS);
        vTaskDelay(pdMS_TO_TICKS(GT911_RETRY_WAIT_MS));
    }
    if (probe_ret != ESP_OK) {
        ESP_LOGE(TAG, "GT911 not found on I2C0 after %d attempts (tried 0x5D and 0x14)",
                 GT911_INIT_ATTEMPTS);
        return probe_ret;
    }
    ...
    esp_lcd_touch_io_gt911_config_t gt911_driver_cfg = {
        .dev_addr = tp_io_cfg.dev_addr,
    };
    esp_lcd_touch_config_t tp_cfg = {
        .x_max        = BSP_LCD_H_RES,
        .y_max        = BSP_LCD_V_RES,
        /* BSP owns the reset pulse and startup wait.  Passing GPIO23 to the
         * generic driver would make it issue its shorter 10 ms reset again
         * immediately before the first configuration read. */
    ...
    for (int attempt = 1; attempt <= GT911_INIT_ATTEMPTS; ++attempt) {
        ...
        touch_ret = esp_lcd_new_panel_io_i2c(s_touch_i2c_bus, &tp_io_cfg, &tp_io);
        if (touch_ret == ESP_OK) {
            touch_ret = esp_lcd_touch_new_i2c_gt911(tp_io, &tp_cfg, &tp);
        }
        if (touch_ret == ESP_OK) {
            uint8_t gt_cfg[5] = {0};
            ...
            if (indev) {
                ESP_LOGI(TAG, "Touch registered: GT911 landscape input device (attempt %d)", attempt);
                return ESP_OK;
            }
            touch_ret = ESP_FAIL;
        }
        ...
        if (touch_ret != ESP_OK) {
            ESP_LOGW(TAG, "GT911 init attempt %d/%d failed: %s", attempt,
                     GT911_INIT_ATTEMPTS, esp_err_to_name(touch_ret));
        }
        ESP_RETURN_ON_ERROR(gt911_hardware_reset(), TAG, "GT911 retry reset failed");
        vTaskDelay(pdMS_TO_TICKS(GT911_RETRY_WAIT_MS));
    }

    ESP_LOGE(TAG, "GT911 initialization failed after %d attempts: %s",
             GT911_INIT_ATTEMPTS, esp_err_to_name(touch_ret));
    return touch_ret;
}
